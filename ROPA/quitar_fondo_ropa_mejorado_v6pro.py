#!/usr/bin/env python3
"""
quitar_fondo_ropa_v6_pro.py

Basado completamente en la versión 6:
- Modelo de IA BiRefNet especializado en prendas (resuelve camisetas blancas en fondos claros).
- Limpieza morfológica y erosión fina para eliminar restos entre piernas.
...
def perfeccionar_mascara(alpha_mask):
    """
    Erosión ligera y limpieza basada estrictamente en la lógica del código 6.
    """
    _, binaria = cv2.threshold(alpha_mask, 50, 255, cv2.THRESH_BINARY)

    # Conservar el componente principal
    ...
    # Erosión morfológica para afinar el contorno
...
def eliminar_sombra_dura(img_bgr, alpha_mask):
    """
    Algoritmo de división de iluminación del código 6.
    """
...
def procesar_imagen(ruta_entrada, carpeta_destino, nombre_base, session, tamano_lienzo=(1000, 1300)):
    ...
    # 2. Perfeccionar máscara (Código 6)
    alpha_mask = perfeccionar_mascara(alpha_raw)

    # 3. Eliminar sombras duras (Código 6)
    ...
    if bbox:
        ...
        raise ValueError("No se pudo detectar la prenda.")
...
def main():
    parser = argparse.ArgumentParser(description="Procesador v6 optimizado para e-commerce.")
    ...
    # Cargar modelo especializado que no falla en blanco sobre blanco
    print("Cargando modelo especializado en ropa (birefnet-clothing)...")
    try:
        session = new_session("birefnet-clothing")
    except Exception:
        # Respaldo si esa versión exacta de rembg no incluye el alias
